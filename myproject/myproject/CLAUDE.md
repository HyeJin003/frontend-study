...
- **성장 목표 (추가됨):**
  - 코드 품질과 개발 역량을 체계적으로 향상
  - 개념과 활용법을 이해하고 직접 적용
  - 실무 수준 문제 해결에 도전 → 멘토는 코드 리뷰와 피드백 제공
  - AI 친화 개발자로 성장 (AI 에이전트, Claude API 활용)
  - 백엔드·DevOps·AI 역량 확보
  - 아키텍처 설계부터 무중단 배포까지

- **멘토링 사이클 (업그레이드):**
  ```
  개념 설명 → 직접 구현 → 코드 리뷰 → 개선
  ```
  점점 본인이 짜고 멘토는 리뷰 중심으로 전환
...
🚧 진행 예정 (Haroom 핵심 기능)
  ...
  3단계  Friendship             친구 신청/수락/거절/취소 (PENDING → ACCEPTED)
  4단계  Scheduler              랜덤매칭 만료 + (TimeCapsule 방향 결정 후) 자동공개
  5단계  Post 마무리            인기글 TOP5
  6단계  결제/구독              PG 결제 (카카오페이/토스), 구독 플랜 (무료/유료)
  7단계  관리자 페이지          회원 관리, 통계 (가입자/결제/활성유저), 구독 현황
  8단계  프론트엔드             React + API 연결 (미니홈피, 랜덤매칭, 친구, 관리자)
  9단계  채팅                   WebSocket + STOMP (랜덤매칭 상대와 실시간 채팅)
  10단계 AI 에이전트             Claude API + Tool Use (타임캡슐 자동 작성, 대화 주제 추천, 악성 콘텐츠 자동 감지 및 처리)
  11단계 배포 환경              Docker + Docker Compose 배포 환경 구축
  12단계 CI/CD                  GitHub Actions 파이프라인 + Docker 기반 무중단 배포 (포트 스위칭 방식)
  13단계 모니터링/로깅          CloudWatch 등 모니터링 및 로깅 시스템 구축
  14단계 스케일링               Kubernetes 기반 오토스케일링
```
