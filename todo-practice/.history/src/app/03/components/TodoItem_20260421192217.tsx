...
export default function TodoItem({ todo }: TodoItemProps) {
  ...
  return (
    ...
    aria-label={`${todo.text}완료표시`}   className="h-4 w-4 cursor-pointer rounded border-gray-300 accent-blue-500"/>

{isEditing ? (<input type='text' value={editText} onChange={event =>setEditText(event.target.value)}
onKeyDown={handleEditKeyDown}
onBlur={handleEditConfirm}
autoFocus
maxLength={200}\  className="flex-1 rounded border border-blue-400 px-2 py-0.5 text-sm
...
<button onClick={()=> deleteTodo(todo.id)} aria-label={`${todo.text}삭제`}
  className='shrink-0  rouded p-1 text-gray-400  transition-colors hover:bg-red-50 hover:text-red-500
